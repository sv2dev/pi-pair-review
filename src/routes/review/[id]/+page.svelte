<script lang="ts">
	...
	type AnnotationDraft = { id?: string; scope: 'global' | 'file' | 'line'; file?: string; line?: number; side?: 'additions' | 'deletions'; body: string };
	type MarkdownEditorApi = { insertText: (text: string) => void; focus: () => void };
	type DiffViewerApi = { scrollFile: (direction: 1 | -1) => void; scrollComment: (direction: 1 | -1) => void; scrollHunk: (direction: 1 | -1) => void; editActiveComment: () => boolean; currentFile: () => string | undefined };
	...
	let lastLineAnnotationTarget = $state<{ scope: 'line'; file: string; line: number; side: 'additions' | 'deletions' } | undefined>();
	...
	let reviewedFileCount = $derived(files.filter((file) => reviewed.has(file.path)).length);
	let reviewProgress = $derived(files.length ? Math.round((reviewedFileCount / files.length) * 100) : 0);
	let allFilesReviewed = $derived(files.length > 0 && reviewedFileCount >= files.length);
	...
	let reviewSettingsJson = $derived(JSON.stringify({ modelKey: agentModelKey, thinkingLevel: agentThinkingLevel, suggestComments, autoReview, reviewLevel: Number(reviewLevel), isolatedLevel, diffStyle, wrap }));
	...
	function restoreReviewed(id: string) {
		...
		try {
			...
			reviewed.clear();
			for (const file of stored ? JSON.parse(stored) as string[] : []) reviewed.add(file);
		} catch {
			reviewed.clear();
		}
	}
	...
	function toggleReviewed(file: string) {
		...
		if (reviewed.has(file)) reviewed.delete(file);
		else reviewed.add(file);
		if (!wasComplete && files.length > 0 && files.every((item) => reviewed.has(item.path))) celebrateCompletion();
	...
	<div class="grid min-h-screen grid-cols-1 grid-rows-[auto_1fr] {gridColumnsClass}">
		<header class="sticky top-0 z-40 col-span-full flex items-center justify-between gap-4 border-b border-border bg-bg/95 px-4 backdrop-blur-sm relative" style="min-height: var(--topbar-height)">
			...
			<div class="flex items-center gap-2">
				...
				<div class="relative">
					<button class="w-8 px-0" title="View options" aria-label="View options" onclick={() => (viewMenuOpen = !viewMenuOpen)}><MoreVertical size={17} /></button>
					{#if viewMenuOpen}
		...
		<aside class="grid content-start gap-2 border-b border-border bg-bg p-2.5 lg:sticky lg:top-[var(--topbar-height)] lg:h-[calc(100vh-var(--topbar-height))] lg:overflow-auto lg:border-b-0 lg:border-r">
			...
			<section class="grid min-w-0 gap-2 overflow-hidden rounded-lg border border-border bg-surface p-2.5">
				...
				<FileTreeViewer files={visibleFiles} {selectedFile} {activeFile} {reviewed} onSelect={(file) => (selectedFile = file)} />
			</section>
		</aside>
		{/if}

		<main class="min-w-0 max-w-full px-3 py-3" style="scroll-padding-top: calc(var(--topbar-height) + 1rem)">
			<DiffViewer bind:this={diffViewer} {session} {findings} {hunkRanks} {reviewLevel} {isolatedLevel} {reviewed} {selectedFile} {targetFindingId} {targetAnnotationId} {diffStyle} {wrap} onActiveChange={(detail) => { activeFile = detail.file; }} onAnnotate={startLineAnnotation} onFileComment={(file) => (annotationDraft = { scope: 'file', file, body: '' })} onToggleReviewed={toggleReviewed} onEditAnnotation={editAnnotation} onDeleteAnnotation={removeAnnotation} />
		</main>
		...
		{#if rightOpen}
			<aside class="grid content-start gap-2 border-t border-border bg-bg p-2.5 lg:sticky lg:top-[var(--topbar-height)] lg:h-[calc(100vh-var(--topbar-height))] min-w-0 overflow-x-hidden lg:overflow-y-auto lg:border-t-0 lg:border-l">
				<section class="grid min-w-0 gap-2 overflow-hidden rounded-lg border border-border bg-surface p-2.5">
					<button class="flex w-full items-center justify-between gap-3 border-0 bg-transparent p-0 hover:bg-transparent" onclick={() => (aiOpen = !aiOpen)}><h2 class="flex items-center gap-1.5 text-[0.85rem] font-semibold">{#if aiOpen}<ChevronDown size={15} />{:else}<ChevronRight size={15} />{/if}AI review</h2><span class="rounded-full bg-code px-1.5 py-0.5 text-[0.66rem] font-semibold uppercase text-muted">{hasAgentReview ? 'done' : session.preReview.status === 'running' ? 'running' : session.preReview.status === 'failed' ? 'failed' : 'not run'}</span></button>
					{#if aiOpen}
						<div class="grid grid-cols-[minmax(0,1fr)_auto] gap-2">
							<button title="AI review settings" onclick={() => (modelDialog = true)}>{#if session.preReview.status === 'running'}<span class="inline-block h-3.5 w-3.5 flex-none animate-spin rounded-full border-2 border-accent/25 border-t-accent"></span>{:else}<Settings size={15} />{/if}Model: {selectedAgentModel?.name ?? 'none'}</button>
				...
				{#if hunkRanks.length > 0}
					<section class="relative grid min-w-0 gap-2 overflow-hidden rounded-lg border border-border bg-surface p-2.5 {reviewModeOpen || reviewLevelOptions.length <= 1 ? '' : 'pb-5'}">
						<div class="flex items-center justify-between gap-3">
							<button class="min-w-0 flex-1 justify-start border-0 bg-transparent p-0 hover:bg-transparent" onclick={() => (reviewModeOpen = !reviewModeOpen)}><h2 class="flex items-center gap-1.5 text-[0.85rem] font-semibold">{#if reviewModeOpen}<ChevronDown size={15} />{:else}<ChevronRight size={15} />{/if}Review mode</h2><span class="truncate text-xs text-muted">{currentReviewMode.label}</span></button>
						</div>
						{#if !reviewModeOpen && reviewLevelOptions.length > 1}
							<div class="absolute inset-x-2.5 bottom-2 flex gap-1" title="Review mode (+/-)">
								{#each reviewLevelOptions as option (option.level)}
									<button class="h-1.5 min-w-0 flex-1 rounded-full border-0 p-0 {(isolatedLevel ? option.level === Number(reviewLevel) : option.level <= Number(reviewLevel)) ? 'bg-accent hover:bg-accent' : 'bg-surface-2 hover:bg-surface-hover'}" aria-label={`Review level ${option.level}`} onclick={() => (reviewLevel = option.level)}></button>
								{/each}
							</div>
						{/if}
						{#if reviewModeOpen}
				...
				<section class="grid min-w-0 gap-2 overflow-hidden rounded-lg border border-border bg-surface p-2.5">
					...
					{#if userAnnotations.length === 0}<p class="text-sm text-muted">Click line numbers, add file notes, or comment overall.</p>{:else}<div class="grid gap-2">{#each userAnnotations as annotation (annotation.id)}<div class="grid gap-1.5 rounded-lg p-2 transition-colors {highlightedEntryId === annotation.id ? 'bg-accent-soft ring-1 ring-accent' : 'bg-surface-2 hover:bg-surface-hover'}"><div class="flex items-center gap-1.5"><button class="min-w-0 flex-1 justify-start border-0 bg-transparent p-0 text-left hover:bg-transparent" onclick={() => selectAnnotation(annotation)}><span class="flex min-w-0 items-center gap-2"><small class="min-w-0 truncate text-muted">{annotation.scope === 'global' ? 'Overall' : annotation.scope === 'file' ? annotation.file : `${annotation.file}:${annotation.line}`}</small></span></button><button class="flex-none rounded-full border-0 bg-transparent px-1 text-muted hover:bg-surface-hover hover:text-fg" title="Edit" aria-label="Edit annotation" onclick={() => editAnnotation(annotation)}><Edit3 size={14} /></button><button class="flex-none rounded-full border-0 bg-transparent px-1 text-muted hover:bg-danger-soft hover:text-danger" title="Remove" aria-label="Remove annotation" onclick={() => removeAnnotation(annotation)}><Trash2 size={14} /></button></div><button class="block w-full justify-start border-0 bg-transparent p-0 text-left hover:bg-transparent" onclick={() => selectAnnotation(annotation)}><div class="rendered-markdown text-sm">{@html renderMarkdown(annotation.body)}</div></button></div>{/each}</div>{/if}
				</section>
			</aside>
		{/if}
	</div>
...
{#if modelDialog}
	<div role="presentation" class="modal-backdrop fixed inset-0 z-40 grid place-items-center p-4" onclick={(event) => backdropClick(event, () => (modelDialog = false))}>
		<div role="dialog" aria-modal="true" aria-labelledby="model-dialog-title" class="grid w-[min(34rem,calc(100vw-2rem))] gap-3 rounded-xl border border-border bg-surface p-5 shadow-[0_16px_48px_var(--shadow)]">
			...
			<div class="flex justify-end gap-2 pt-1"><button title="Close dialog (Esc)" type="button" onclick={() => { persistReviewSettings(); modelDialog = false; }}>Close</button><button class="border-accent bg-accent text-accent-fg hover:bg-accent hover:opacity-90" title="Run AI review" type="button" disabled={!agentModelKey || session?.preReview.status === 'running'} onclick={runAgentReview}><Play size={15} />Run</button></div>
		</div>
	</div>
{/if}

{#if summaryDialog}
	<div role="presentation" class="modal-backdrop fixed inset-0 z-40 grid place-items-center p-4" onclick={(event) => backdropClick(event, () => (summaryDialog = false))}>
		<div role="dialog" aria-modal="true" aria-labelledby="summary-dialog-title" class="grid max-h-[min(70vh,42rem)] w-[min(40rem,calc(100vw-2rem))] gap-3 overflow-auto rounded-xl border border-border bg-surface p-5 shadow-[0_16px_48px_var(--shadow)]">
			...
			<div class="flex justify-end gap-2 pt-1"><button type="button" onclick={() => (summaryDialog = false)}>Close</button></div>
		</div>
	</div>
{/if}

{#if completionDialog}
	<div role="presentation" class="modal-backdrop fixed inset-0 z-40 grid place-items-center p-4" onclick={(event) => backdropClick(event, () => (completionDialog = false))}>
		<div class="confetti-layer" aria-hidden="true">{#each Array.from({ length: 24 }, (_, index) => index) as index (index)}<i></i>{/each}</div>
		<div role="dialog" aria-modal="true" aria-labelledby="completion-dialog-title" class="relative z-10 grid w-[min(34rem,calc(100vw-2rem))] gap-3 rounded-xl border border-accent/50 bg-surface p-5 shadow-[0_16px_48px_var(--shadow)]">
			...
			<div class="flex justify-end gap-2 pt-1"><button type="button" onclick={() => (completionDialog = false)}>Keep reviewing</button><button class="border-accent bg-accent text-accent-fg hover:bg-accent hover:opacity-90" type="button" onclick={() => finish()}><Check size={15} />Insert feedback</button></div>
		</div>
	</div>
{/if}

{#if shortcutsDialog}
	<div role="presentation" class="modal-backdrop fixed inset-0 z-40 grid place-items-center p-4" onclick={(event) => backdropClick(event, () => (shortcutsDialog = false))}>
		<div role="dialog" aria-modal="true" aria-labelledby="shortcuts-dialog-title" class="grid w-[min(32rem,calc(100vw-2rem))] gap-3 rounded-xl border border-border bg-surface p-5 shadow-[0_16px_48px_var(--shadow)]">
			...
			<div class="flex justify-end gap-2 pt-1"><button type="button" onclick={() => (shortcutsDialog = false)}>Close</button></div>
		</div>
	</div>
{/if}

{#if annotationDraft}
	<div role="presentation" class="modal-backdrop fixed inset-0 z-40 grid place-items-center p-4" onclick={(event) => backdropClick(event, () => (annotationDraft = undefined))}>
		<div role="dialog" aria-modal="true" aria-labelledby="annotation-dialog-title">
			<form class="grid w-[min(36rem,calc(100vw-2rem))] gap-3 rounded-xl border border-border bg-surface p-5 shadow-[0_16px_48px_var(--shadow)]" onsubmit={submitAnnotation}>
				...
				<div class="flex justify-end gap-2 pt-1"><button title="Close dialog (Esc)" type="button" onclick={() => (annotationDraft = undefined)}>Cancel</button><button class="border-accent bg-accent text-accent-fg hover:bg-accent hover:opacity-90" title="Save comment (Ctrl+S or Cmd+Enter)" type="submit">Save</button></div>
			</form>
		</div>
	</div>
{/if}
